...
presentation.add_slide(
    f"""<h2 style="text-align:center;">計算機程式概論課程<br>可攜式系統組成內容介紹</h2>
<style>
...
<div style="text-align:left; font-size:{font_size};">
...
# 第二張投影片（fragment）
presentation.add_slide(
    f"""<h2 style="text-align:center;">Git是什麼</h2>
<style>
@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@400;700&display=swap');
body, h1, h2, h3, p, div {{
    font-family: 'Noto Sans TC', sans-serif;
...
# 第三張投影片（GitHub是什麼）
presentation.add_slide(
    f"""<h2 style="text-align:center;">GitHub是什麼?</h2>
<style>
@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@400;700&display=swap');
body, h1, h2, h3, p, div {{
    font-family: 'Noto Sans TC', sans-serif;
...
# 其餘投影片
slides_content = [
    ("Gits是什麼", "Gits 用來快速分享程式碼片段、筆記或任何文字檔，並且只要有連結就可以快速地將檔案分享給其他人。<br>常被用來放程式碼範例、快速筆記、或是分享設定檔。"),
    ("Cmsimde是什麼", "CMSiMDE 是一套以 Python 和 Flask 框架開發的網際內容管理系統，專注於機械設計領域，可管理數位內容，並具有部落格與網頁簡報製作等功能。"),
    ("如何使用這些工具?", "1. 建立 GitHub 帳號以方便使用 Gits<br>2. 在近端建立 Git 並連結 GitHub<br>3. 透過 Cms 設定 Git 和 GitHub 間的溝通管道<br>4. 接著將 GitHub 資料庫網址存下並連結至 Cmsimde<br>5. 日後只要用 Cms 查詢網路，推送到遠端：<br>　git push origin main <br>　暫存修改：git add .<br>　提交修改：git commit -m ''<br>　檢查工作目錄狀態：git status<br>基本上就完成了。"),
    ("為什麼要使用這些工具?", "在製作近端及遠端的過程中，我們清楚明白地理解到，一旦建立好近端可攜系統後，日後只要攜帶個 USB 或 SSD 固態硬碟就可以隨時隨地立即修改程式，再將程式連結 GitHub 就可以輕鬆更改程式，在這過程中甚至還有 Git 紀錄修改的過程，就連 Cmsimde 在修改的過程也會錄下來。"),
    ("資料索引", "少量GPT<br>集結組員知識")
]
